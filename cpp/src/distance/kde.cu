...
template <typename T>
struct DistOp<T, cuvs::distance::DistanceType::L2SqrtUnexpanded> {
...
template <typename T>
struct DistOp<T, cuvs::distance::DistanceType::L2Expanded> {
  static constexpr int N_ACC = 1;
  inline __device__ static void init(T* acc) { acc[0] = T(0); }
  inline __device__ static void accumulate(T* acc, T a, T b, T)
  {
...
// manhattan: sum(|a-b|)
template <typename T>
struct DistOp<T, cuvs::distance::DistanceType::L1> {
  static constexpr int N_ACC = 1;
  inline __device__ static void init(T* acc) { acc[0] = T(0); }
...
// chebyshev: max(|a-b|)
template <typename T>
struct DistOp<T, cuvs::distance::DistanceType::Linf> {
  static constexpr int N_ACC = 1;
  inline __device__ static void init(T* acc) { acc[0] = T(0); }
  inline __device__ static void accumulate(T* acc, T a, T b, T)
  {
...
template <typename T>
struct DistOp<T, cuvs::distance::DistanceType::LpUnexpanded> {
  static constexpr int N_ACC = 1;
  inline __device__ static void init(T* acc) { acc[0] = T(0); }
...
template <typename T>
struct DistOp<T, cuvs::distance::DistanceType::CosineExpanded> {
...
template <typename T>
struct DistOp<T, cuvs::distance::DistanceType::CorrelationExpanded> {
...
template <typename T>
struct DistOp<T, cuvs::distance::DistanceType::Canberra> {
  static constexpr int N_ACC = 1;
  inline __device__ static void init(T* acc) { acc[0] = T(0); }
  inline __device__ static void accumulate(T* acc, T a, T b, T)
  {
...
template <typename T>
struct DistOp<T, cuvs::distance::DistanceType::HellingerExpanded> {
  static constexpr int N_ACC = 1;
  inline __device__ static void init(T* acc) { acc[0] = T(0); }
...
// jensen-shannon
template <typename T>
struct DistOp<T, cuvs::distance::DistanceType::JensenShannon> {
  static constexpr int N_ACC = 1;
  inline __device__ static void init(T* acc) { acc[0] = T(0); }
  inline __device__ static void accumulate(T* acc, T a, T b, T)
  {
...
// hamming: count(a!=b)/d
template <typename T>
struct DistOp<T, cuvs::distance::DistanceType::HammingUnexpanded> {
  static constexpr int N_ACC = 1;
  inline __device__ static void init(T* acc) { acc[0] = T(0); }
...
template <typename T>
struct DistOp<T, cuvs::distance::DistanceType::KLDivergence> {
  static constexpr int N_ACC = 1;
  inline __device__ static void init(T* acc) { acc[0] = T(0); }
  inline __device__ static void accumulate(T* acc, T a, T b, T)
  {
...
template <typename T>
struct DistOp<T, cuvs::distance::DistanceType::RusselRaoExpanded> {
  static constexpr int N_ACC = 1;
  inline __device__ static void init(T* acc) { acc[0] = T(0); }
  inline __device__ static void accumulate(T* acc, T a, T b, T) { acc[0] += a * b; }
...
// ============================================================================
// Log-kernel traits — one specialisation per DensityKernelType
// ============================================================================

template <typename T, DensityKernelType K>
struct LogKernel;

template <typename T>
struct LogKernel<T, DensityKernelType::Gaussian> {
  inline __device__ static T eval(T x, T h) { return -(x * x) / (T(2) * h * h); }
};

template <typename T>
struct LogKernel<T, DensityKernelType::Tophat> {
...
template <typename T>
struct LogKernel<T, DensityKernelType::Epanechnikov> {
  inline __device__ static T eval(T x, T h)
  {
...
template <typename T>
struct LogKernel<T, DensityKernelType::Exponential> {
  inline __device__ static T eval(T x, T h) { return -x / h; }
};

template <typename T>
struct LogKernel<T, DensityKernelType::Linear> {
  inline __device__ static T eval(T x, T h)
  {
...
template <typename T>
struct LogKernel<T, DensityKernelType::Cosine> {
  inline __device__ static T eval(T x, T h)
  {
...
template <typename T>
T norm_factor(DensityKernelType kernel, T h, int d)
{
  T factor;
  switch (kernel) {
    ...
    case DensityKernelType::Cosine: {
      // Compute integral_0^1 cos(pi/2 * t) * t^{d-1} dt using the recurrence:
      //   I_n = (2/pi) - n*(n-1)*(2/pi)^2 * I_{n-2}
      //   I_0 = 2/pi,  I_1 = 2/pi - (2/pi)^2
      ...
      if (n == 0) {
        ...
        for (int j = 2; j <= n; ++j) {
          T I_next = two_over_pi - T(j) * T(j - 1) * two_over_pi_sq * I_prev;
          I_prev   = I_curr;
          I_curr   = I_next;
        }
        factor = std::log(I_curr) + logSn<T>(d - 1);
      }
    } break;
...
void dispatch_metric(cuvs::distance::DistanceType metric, Fn&& fn)
{
...
template <typename T, typename Fn>
void dispatch_kernel(DensityKernelType kernel, Fn&& fn)
{
  switch (kernel) {
    case DensityKernelType::Gaussian:
      fn(std::integral_constant<DensityKernelType, DensityKernelType::Gaussian>{});
      break;
    case DensityKernelType::Tophat:
      fn(std::integral_constant<DensityKernelType, DensityKernelType::Tophat>{});
      break;
    case DensityKernelType::Epanechnikov:
      fn(std::integral_constant<DensityKernelType, DensityKernelType::Epanechnikov>{});
      break;
    case DensityKernelType::Exponential:
      fn(std::integral_constant<DensityKernelType, DensityKernelType::Exponential>{});
      break;
    case DensityKernelType::Linear:
      fn(std::integral_constant<DensityKernelType, DensityKernelType::Linear>{});
      break;
    case DensityKernelType::Cosine:
      fn(std::integral_constant<DensityKernelType, DensityKernelType::Cosine>{});
      break;
